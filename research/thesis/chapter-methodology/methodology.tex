...
An overview of the modeling and the computational methods used for the investigation of flows past square or similarly buff cross sections in this study are presented in this chapter. This study uses well established techniques to model and study fluid elastic galloping. Therefore, only a brief overview is provided together with relevant references where the development and vigorous validation has been presented.  
...
These assumptions lead to the Navier-Stokes equations as the equations which govern the motion of a Newtonian, incompressible fluid.
  ...
  These equations are generalised by non-dimensionalisation. In the case of bluff body wake flows, the equations are typically non-dimensionalised by using the characteristic length of the body i.e the frontal projected height \diam, and the free-stream velocity \Ufree. 
  ...
  \begin{equation} \centering
  \label{eq:cyl_eom}
  \frac{\ddot{y}_{body}}{D} + 2\damping\sqrt{\kstar}\frac{\dot{y}_{body}}{D} + \kstar\frac{\ycyl}{D} = \frac{\pi}{2}\frac{\clift}{\mstar} ~,
  \end{equation}
...
\input{./chapter-methodology/figures/convection-sketch.tex}
...
The body velocity approximation $\Vcyl^{\dag}$, was updated commencing the ``corrector'' cycle of the predictor-corrector method. This was carried out using a third-order integration scheme. 
 ...
 \frac{\Vcyl^{(n+1)\dag} - \Vcyl^{(n)}}{\Delta\tau} = \frac{1}{24}(25\ddot{y}_{body}^{(n+1)} - 2\ddot{y}_{body}^{(n)} + \ddot{y}_{body}^{(n-1)}) ~.
 \end{equation}
