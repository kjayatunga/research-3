...
While the velocity and power data collapse well, the amplitude data still shows some spread. Figure \ref{fig:amp-collapsed} shows the displacement amplitude data obtained in figure \ref{fig:compare_data} (a), but rescaled on a length scale that considers the stiffness by incorporating \massstiff. This is due to the fact that the amplitude of the system is fixed by the balance between the maximum induced lift force and the spring




% The amplitude though is fixed by a balance between the maximum lift force and the spring; the lift can only elongate the spring so far; stiffer springs have less amplitude, and so the amplitude scales with 1/stiffness, or 1/Pi_1.
