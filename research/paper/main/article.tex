...
where $F_y$ is the transverse (lift) force.
 ...
 % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
   ...
   Figure \ref{fig:compare_data} shows the comparison of mean power data presented using different variables. Sub-figure (a) shows data obtained using classical VIV parameters $\ustar$ and $\zeta$. The data follows the same trends with \cite{Barrero-Gil2010a}. The same data is collapsed in sub-figure (b) using \massdamp as the independent variable. The collapse is excellent. 

 \input{../figure/collapsed_data}
    
         
     The quasi-steady analysis data reveal that the displacement amplitude grows with decreasing \massdamp \ . This is shown for both the high and low \reynoldsnumber\ cases in figures \ref{fig:collapsed_data} (a) and (b). Hysteresis could be observed for the case with a higher Reynolds number. Different solutions could be obtained by manipulating the initial conditions (initial displacement) of the system. The upper branch was obtained by giving an initial displacement which was higher than the expected amplitude while the lower branch was obtained by providing a lower initial displacement than the expected amplitude. Although theory shows a possible third state, it is an unstable branch and as such it could not be achieved numerically. This was also observed by \cite{Vio2007}. 
 ...
 \subsection{Displacement, velocity and power}
    ...
    \input{../figure/power_curve_sketch}
...
\citet{Joly2012} showed that the displacement data obtained using the QSS assumption and DNS agree well at low Reynolds numbers, with the modification implemented to the oscillator equation which accounts for the vortex shedding. These data were obtained at zero damping levels. Since the current study was focused on the power transfer,  analysing the behaviour of the velocity as the damping is increased was of interest. The comparison between the QSS and the DNS results (figure \ref{fig:qss_fsi}) revealed that
the discrepancy of the power data  increases significantly as \massdamp \ is increased for a given \massstiff \. However, it could be also be observed that the discrepancy on power is reduced when the mass ratio is increased. The percentage error of power data between DNS and QSS were evaluated using equation\ref{eqn:error_calculation} and is presented in table \ref{table:error}. The error seems to decrease as \massstiff is increased. Since \massdamp \ was increased by changing the \mstar and keeping \ustar constant, it is evident that as the inertia of the system is increased the error decreases even at higher \massdamp. The power spectrum data presented in \hilight{the figure} reveals that influence of vortex shedding is more prominent at low mass ratio therefore results in deviating from quasi-steady state results. This strengthens \cite{Joly2012} conclusions. However, the significant of this study with that of \cite{Joly2012} is that we have provided a comparison of QSS and DNS data as damping is increased. 
