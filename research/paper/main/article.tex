...
%\end{equation}
 ...
 \KJ{Kenny's openion is that we need to justify this by saying fixed amplitude and fixed frequency does not accurately predict shedding forces (I need some references for this. Do you have any since you have been doing VIV research for a square cross section). What we have to say that what the sinusoidal function joly used does not apply to a universal case. i.e. No single value can accurately model the shedding forces across a brorad range of parameters with damping.} 
...
\subsection{Comparison of power between high and low \reynoldsnumber data}   
 ...
 Figure \ref{fig:collapsed_data} 
 \KJ{Have to write something about power for both high and low reynolds number not exactly sure what to write}
...
\subsection{High \ \massstiff}

\input{../figure/high_pi_1}

For definition purposes $\massstiff \geq 10$  is defined as high \massstiff \ and the rest as low \massstiff in this article. Figure \ref{fig:high_pi_1} (a)  shows that the mean power and the velocity is a strong function of \massdamp. The collapse of velocity amplitude and the mean power is excellent. Figure \ref{fig:high_pi_1} (a) shows that the power is independent of \massstiff and \mstar. The relationship between power and \massdamp could be explained by analysing the time histories of selected cases. Data at $\massstiff=10$, $m^*=20$ and $\reynoldsnumber=200$ are shown in figure \ref{fig:power_time_histories} and are analysed as an example. 

 
 The instantaneous power from the fluid to the body can be expressed as $P_t=F_y\dot{y}$. Similarly the dissipated power due to the mechanical damping can be expressed as $P_d=(c\dot{y})\dot{y}$. The time average of these two quantities, described in equations \ref{power} and \ref{power_alt} should be equal due to energy conservation, provided that the mechanical friction losses are neglected. The mean power vs $\massdamp$ (Figure \ref{fig:high_pi_1}) provides a detailed explanation for the variation of the output power when \massdamp \ is increased. The key regions consists of region 1 where the $P_{mean}$ increases with \massdamp, region 2 where $P_{mean}$ becomes maximum and region 3 where $P_{mean}$ decreases with \massdamp. Time histories of $P_t $ and $P_d$ at each of these key regions are presented in figure \ref{fig:power_time_histories}.
 ...
 \subsection{Low \massstiff}
 

 \input{../figure/low_pi_1}
 
  As \massstiff \ reduces, at low \massstiff (Fig:\ref{fig:high_pi_1}) (b), the power increases indicating that power is a weak function of \massstiff \ at low \ \massstiff levels. The reason behind this is as \massstiff reduces, the response of the system moves towards a square signal from a sinusoidal signal. This is due to the fact that in this region the forcing is dominated by the higher order terms in the forcing function (Eq:\ref{cy ploynomial}).  The formulation of \massstiff \ and \massdamp \ does not include the non-linear terms of Eq: \ref{eqn:eom_linear}. However,\mstar is not eliminated in scaling the higher-order terms. Figure \ref{fig:low_pi_1} proves that even at this region the power is independent of \mstar due to the facet that the power data does not change as \mstar is varied for a give \massstiff.  Yet, this particular region of low \massstiff \ could not be captured in the DNS results as the vortex shedding is much stronger and overcomes the galloping signal indicating that the QSS hypothesis deviates from the actual behaviour of the system at low \massstiff.
...
\citet{Joly2012} showed that the displacement data obtained using the QSS assumption and DNS agree well at low Reynolds numbers, with the modification implemented to the oscillator equation which accounts for the vortex shedding. These data were obtained at zero damping levels. However, the current study was focused on the behaviour and the power transfer of the system. Therefore analysing the behaviour of the system with increasing damping is of interest.

The comparison between QSS and the DNS results (Fig:\ref{fig:qss_fsi} (c)) revealed that the mean power has influence of both \massstiff \ and \ \massdamp. The discrepancy reduced as the \massdamp was decreased and the \massstiff was increased. However, \massstiff has a much more influence on the power extracted than predicted by the QSS model. 



\input{../figure/max_power}

The percentage error was quantified using Eq:\ref{eqn:error_calculation} between the QSS and DNS data. Figure \ref{fig:error} indicates that the error is an inverse function of \ \massstiff. 
...
\input{../figure/spectrum}
 ...
 The power spectrum of the velocity signal of the system presented in figure \ref{fig:spectrum}, shows the  velocity signals at $\massstiff=0.8$ and $\massdamp= 10, 60, 250$ and $1000$ and the corresponding spectrum. The spectral data shows a significant response at $\frac{fd}{U}=0.156$ which could be identified as the vortex shedding frequency. The magnitude  reduces as \massstiff \ is increased. Hence, indicating the influence of vortex shedding is more prominent at low \massstiff therefore results in larger deviations from quasi-steady state results. This strengthens \cite{Joly2012} conclusions. However, the significant of this study with that of \cite{Joly2012} is that  it has provided a comparison of QSS and DNS data as damping is increased. 

The error between the predicted maximum power between QSS and DNS (Fig:\ref{fig:error}) shows that it scales similar to a $\frac{1}{\massstiff}$ function. As previously explained, this is due to the influence of vortex shedding.  
...
\input{../figure/spec_pow}
 ...
 \section{Conclusion}
  ...
  In comparison with the Direct numerical simulation data, it could be concluded that the QSS model provides a good estimate of the power output of the system when \massdamp \ is relatively high and predicted value of \massdamp \ at maximum power is quite good. The error in predicted maximum power between the QSS and the DNS data scales as an inverse function of \massdamp 
