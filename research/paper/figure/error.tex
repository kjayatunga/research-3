...
%       \put(0.5,0.4){$\massdamp$}
  ...
  % \caption{Comparison of maximum power between QSS and DNS data obtained using 3 point local quadratic curve fitting.The error was obtained using Eq:\ref{eqn:error_calculation}}
    \caption{The percentage error between the maximum power obtained using DNS data and predicted by QSS model as a function of  \massstiff. The QSS model prediction is worst for low values of  \massstiff. The dash curve (\protect\dashedrule) follows the power low fit of the percentage error which is $f(x)=138.697x^{-0.6} $ equation }
    \label{fig:error}
\end{figure}
